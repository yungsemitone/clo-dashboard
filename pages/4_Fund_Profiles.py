...
if st.session_state[summary_key]:
    ...
    # --- Assemble portfolio stats (latest filing) for the summary ---
    priced_latest = latest_df.dropna(subset=["implied_price"])
    above_50 = int((priced_latest["implied_price"] >= 50).sum())
    below_20 = int((priced_latest["implied_price"] < 20).sum())
    median_price = float(priced_latest["implied_price"].median()) if not priced_latest.empty else 0.0

    top_mgr_series = latest_df.groupby("manager")["par_amount"].sum().nlargest(5)
    top_managers = [(name, par / 1e6) for name, par in top_mgr_series.items()]
    top5_pct = (top_mgr_series.sum() / total_par * 100) if total_par > 0 else 0
    ...
    fund_data = {
        ...
        "total_par_mm": total_par / 1e6,
        "total_mv_mm": total_mv / 1e6,
        "avg_price": avg_price,
        "median_price": median_price,
        "top_managers": top_managers,
        "top5_concentration_pct": top5_pct,
        "largest_position": (largest["deal_name"], largest["par_amount"] / 1e6),
        "n_above_50": above_50,
        "n_below_20": below_20,
        "n_priced": len(priced_latest),
    }

    # AI-generated when an API key is present, rule-based otherwise (same module).
    ...
    if ai_summaries_enabled():
        st.caption(f"✨ Generated by Claude ({DEFAULT_SUMMARY_MODEL}) from the NPORT-P portfolio data.")
    else:
        st.caption("Rule-based summary. Set ANTHROPIC_API_KEY to enable AI-generated summaries.")
